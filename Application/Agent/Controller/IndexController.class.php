...
class IndexController extends AgentController
{
	...
	//代理中心建仓订单
	public function tyjclist($username=NULL){
        if (!session('agent_id')) {
			$this->redirect('Agent/Login/index');
		}
		...
		if(!empty($ulist)){
		    ...
		    foreach($ulist as $key=>$vo){
		        $map['uid'] = $vo['id'];
		        $map['status'] = 1;
		        $orderlist = M("tyhyorder")->where($map)->select();
		        if(!empty($orderlist)){
	...
	public function czlist(){
		...
		if(!empty($ulist)){
		    ...
		    foreach($ulist as $k=>$v){
		        $uid = $v['id'];
		        $corder = M("recharge")->where(array('uid'=>$uid))->select();
		        if(!empty($corder)){
		            foreach($corder as $key=>$vo){
		                $list[$key]['id'] = $vo['id'];
		                $list[$key]['username'] = $vo['username'];
		                $list[$key]['coin'] = $vo['coin'];
		                $list[$key]['num'] = $vo['num'];
		                $list[$key]['addtime'] = $vo['addtime'];
		                $list[$key]['updatetime'] = $vo['updatetime'];
		                $list[$key]['status'] = $vo['status'];
		                $list[$key]['payimg'] = $vo['payimg'];
		            }
		        }
	...
	public function txlist(){
		...
		if(!empty($ulist)){
		    ...
		    foreach($ulist as $k=>$v){
		        $uid = $v['id'];
		        $corder = M("myzc")->where(array('userid'=>$uid))->select();
		        if(!empty($corder)){
		            foreach($corder as $key=>$vo){
		                $list[$key]['id'] = $vo['id'];
		                $list[$key]['userid'] = $vo['userid'];
		                $list[$key]['username'] = $vo['username'];
		                $list[$key]['coinname'] = $vo['coinname'];
		                $list[$key]['num'] = $vo['num'];
		                $list[$key]['fee'] = $vo['fee'];
		                $list[$key]['mum'] = $vo['mum'];
		                $list[$key]['address'] = $vo['address'];
		                $list[$key]['addtime'] = $vo['addtime'];
		                $list[$key]['endtime'] = $vo['endtime'];
		                $list[$key]['status'] = $vo['status'];
		            }
		        }
	...
	//代理中心平仓订单
	public function typclist($username=NULL){
        if (!session('agent_id')) {
			$this->redirect('Agent/Login/index');
		}
		...
		if(!empty($ulist)){
		    ...
		    foreach($ulist as $key=>$vo){
		        $uid = $vo['id'];
		        $orderlist = M("tyhyorder")->where("uid = $uid and (status = 2 or status = 3)")->select();
		        if(!empty($orderlist)){
		            foreach($orderlist as $k=>$v){
		                $list[$key][$k]['id'] = $v['id'];
		                $list[$key][$k]['username'] = $v['username'];
