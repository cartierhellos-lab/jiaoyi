...
class AjaxtradeController extends MobileController
{
    ...
    public function processing_onedata($array){
        ...
        $close1 = $price_arr['close1'];//现价 //单独数组
        $lowhig =  $close - $open; //涨跌
        $change = round(($lowhig / $open * 100),2); //涨跌幅
        $rmb = $close * 6.742;
        $rmb = round($rmb,2); //保留两个小数点
        
        $tc_rmb_jiage = $close * 6.742;
        
        
        //单独数组 判断
        if($close1 >= $open){
            $close1 = $tc_rmb_jiage;
            $close1 = '<span id="lv_k">CNY ¥ '.$tc_rmb_jiage.'</span>'; //绿色 
            
        }else{
            $close1 = $tc_rmb_jiage;
            $close1 = '<span id="lv_k">CNY ¥ '.$tc_rmb_jiage.'</span>'; //红色
        }
        
        if($change >= 0){
            $change = '<span class="green"  id="green" style="color: #333;display: inline-block;font-style: normal;font-size: 8pt;">+' .$change. '%</span>'; //绿色
        }else{
            $change = '<span class="red" id="red" style="color: #333;display: inline-block;font-style: normal;font-size: 8pt;">' .$change.'%</span>'; //红色
            
        }
        
        if($close >= $open){
            $close = '<span class="green"  style="color: #333;width: 100%;font-size: 17px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;font-weight: 600;line-height: normal;">'.$close.'<p><span  id="cny" style="color: #999; line-height: 20px; font-size: 10px; text-overflow: ellipsis; display: inline-block; width: 100%; font-weight: 400;overflow: hidden;">¥&nbsp;'.$rmb.'</span></span>'; //绿色
            
        }else{
            $close = '<span class="red" style="color: #333;width: 100%;font-size: 17px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;font-weight: 600;line-height: normal;">'.$close.'<p><span style="color: #999; line-height: 20px; font-size: 10px; text-overflow: ellipsis; display: inline-block; width: 100%; font-weight: 400;overflow: hidden;">¥&nbsp;'.$rmb.'</span></span>'; //红色
        }
    ...
    //         $change = round(($lowhig / $open * 100),2); //涨跌幅
    //         if($change >= 0){
    //             $changestr = "<span  class='fzmm bgreen' style='width: 70px;height: 30px;text-align: center;margin-top: 8px;display: -webkit-box;display: -ms-flexbox;display: flex;-webkit-box-pack: center;-ms-flex-pack: center;justify-content: center;-webkit-box-align: center;-ms-flex-align: center;align-items: center;font-size: 12px;color: #fff!important;float: right;border-radius: 3px; font-weight: 600; line-height: 25px; font-size: 14px;'>+". $change ."%</span>";
    //         }else{
    //             $changestr = "<span  class='fzmm bred' style='width: 70px;height: 30px;text-align: center;margin-top: 8px;display: -webkit-box;display: -ms-flexbox;display: flex;-webkit-box-pack: center;-ms-flex-pack: center;justify-content: center;-webkit-box-align: center;-ms-flex-align: center;align-items: center;font-size: 12px;color: #fff!important;float: right;border-radius: 3px; font-weight: 600; line-height: 25px; font-size: 14px;'>". $change ."%</span>";
    //         }
    ...
    // public function obtain_eth2($coin){
    //     $symbol = $coin."usdt";
    //     $cname = strtoupper($coin)."/USDT";
    //     $api = "https://api.huobi.pro/market/history/kline?period=1day&size=1&symbol=".$symbol;
    //     $data = $this->get_maket_api($api);
    ...
    //         $change = round(($lowhig / $open * 100),2); //涨跌幅
    //         if($change >= 0){
    //             $changestr = "<span  class='f14 fgreen fw'>+". $change ."%</span>";
    //         }else{
    //             $changestr = "<span  class='f14 fred fw'>". $change ."%</span>";
    //         }
    ...
    public function obtain_eos($coin){
      ...
      //获取行情单个行情数据
    public function obtain_xrp($coin){
        $symbol = $coin."usdt";
        $cname = strtoupper($coin)."/USDT";
        $api = "https://api.huobi.pro/market/history/kline?period=1day&size=1&symbol=".$symbol;
        $data = $this->get_maket_api($api);
    ...
    //获取行情单个行情数据
    public function obtain_trx($coin){
        $symbol = $coin."usdt";
        $cname = strtoupper($coin)."/USDT";
        $api = "https://api.huobi.pro/market/history/kline?period=1day&size=1&symbol=".$symbol;
        $data = $this->get_maket_api($api);
    ...
    //获取行情单个行情数据
    public function obtain_sol($coin){
        $symbol = $coin."usdt";
        $cname = strtoupper($coin)."/USDT";
        $api = "https://api.huobi.pro/market/history/kline?period=1day&size=1&symbol=".$symbol;
        $data = $this->get_maket_api($api);
    ...
    //获取行情单个行情数据
    public function obtain_bnb($coin){
        $symbol = $coin."usdt";
        $cname = strtoupper($coin)."/USDT";
        $api = "https://api.huobi.pro/market/history/kline?period=1day&size=1&symbol=".$symbol;
        $data = $this->get_maket_api($api);
    ...
    //获取行情单个行情数据
    public function obtain_wbtc($coin){
        $symbol = $coin."usdt";
        $cname = strtoupper($coin)."/USDT";
        $api = "https://api.huobi.pro/market/history/kline?period=1day&size=1&symbol=".$symbol;
        $data = $this->get_maket_api($api);
     ...
     //获取行情单个行情数据
    public function obtain_dot($coin){
        $symbol = $coin."usdt";
        $cname = strtoupper($coin)."/USDT";
        $api = "https://api.huobi.pro/market/history/kline?period=1day&size=1&symbol=".$symbol;
        $data = $this->get_maket_api($api);
     ...
     //获取行情单个行情数据
    public function obtain_avax($coin){
        $symbol = $coin."usdt";
        $cname = strtoupper($coin)."/USDT";
        $api = "https://api.huobi.pro/market/history/kline?period=1day&size=1&symbol=".$symbol;
        $data = $this->get_maket_api($api);
     ...
     //获取行情单个行情数据
    public function obtain_uni($coin){
        $symbol = $coin."usdt";
        $cname = strtoupper($coin)."/USDT";
        $api = "https://api.huobi.pro/market/history/kline?period=1day&size=1&symbol=".$symbol;
        $data = $this->get_maket_api($api);
     ...
     //获取行情单个行情数据
    public function obtain_etc($coin){
        $symbol = $coin."usdt";
        $cname = strtoupper($coin)."/USDT";
        $api = "https://api.huobi.pro/market/history/kline?period=1day&size=1&symbol=".$symbol;
        $data = $this->get_maket_api($api);
     ...
     //获取行情单个行情数据
    public function obtain_near($coin){
        $symbol = $coin."usdt";
        $cname = strtoupper($coin)."/USDT";
        $api = "https://api.huobi.pro/market/history/kline?period=1day&size=1&symbol=".$symbol;
        $data = $this->get_maket_api($api);
     ...
     //获取行情单个行情数据
    public function obtain_xmr($coin){
        $symbol = $coin."usdt";
        $cname = strtoupper($coin)."/USDT";
        $api = "https://api.huobi.pro/market/history/kline?period=1day&size=1&symbol=".$symbol;
        $data = $this->get_maket_api($api);
     ...
     //获取行情单个行情数据
    public function obtain_ape($coin){
        $symbol = $coin."usdt";
        $cname = strtoupper($coin)."/USDT";
        $api = "https://api.huobi.pro/market/history/kline?period=1day&size=1&symbol=".$symbol;
        $data = $this->get_maket_api($api);
     ...
     //获取行情单个行情数据
        public function obtain_aave($coin){
        $symbol = $coin."usdt";
        $cname = strtoupper($coin)."/USDT";
        $api = "https://api.huobi.pro/market/history/kline?period=1day&size=1&symbol=".$symbol;
        $data = $this->get_maket_api($api);
