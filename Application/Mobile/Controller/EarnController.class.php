...
class EarnController extends MobileController
{
	...
	// @panagen +++++
	public function c2c_username(){
	  $uid = userid(); //获取用户id 
	  // where(array('id'=>1)) 指定获取用户id 
// 	  where(array('id'=>$uid)) 直接获取自身登陆的用户id
	  $uinfo = M("user")->where(array('id'=>1))->field("id,l2c2_username")->find();
	  
	  $fbsj_zc = M("user_coin")->where(array('id'=>1))->field("id,usdt")->find();

	        
	       // $data['uid'] = $uid;
	        $data['l2c2_username'] = $uinfo['l2c2_username'];
	        $data['usdt'] = $fbsj_zc['usdt'];
	       // $data['taxnum'] = $zznum;
	       ...
	       if($uinfo['l2c2_username'] == ""){
	           
	          $this->ajaxReturn(['code'=>0,'usdt'=> L('提交失败')]);
	        }else{
	          $this->ajaxReturn(['code'=>1,'usdt'=>$fbsj_zc['usdt']]);
	        }
	         
	}
	
		// @panagen +++++
	public function c2c_username_name(){
	  $uid = userid(); //获取用户id 
	  $uinfo = M("user")->where(array('id'=>1))->field("id,l2c2_username")->find();
	       ...
	       if($uinfo['l2c2_username'] == ""){
	          ...
	          $this->ajaxReturn(['code'=>0,'l2c2_username'=> L('提交失败')]);
	        }else{
	          $this->ajaxReturn(['code'=>1,'l2c2_username'=>$uinfo['l2c2_username']]);
	        }
	...
	public function transfer(){
	    if($_POST){
	        $tbnum = trim(I('post.tbnum'));
	        $type = trim(I('post.type'));
	        $uid = userid();
	        if($tbnum <= 0){
	            $this->ajaxReturn(['code'=>2,'info'=>L('请输入正确的数量')]);exit();
	        }
	        $uinfo = M("user")->where(array('id'=>$uid))->field("id,username")->find();
	        ...
	        if($type == 1){
	            if($money < $tbnum){
	                $this->ajaxReturn(['code'=>2,'info'=>L('资金账户余额不足')]);exit();
	            }
	            $dec_re = M("user_coin")->where(array('userid'=>$uid))->setDec('usdt',$tbnum);
	            if($dec_re){
	                $inc_re = M("levermoney")->where(array('uid'=>$uid))->setInc('money',$tbnum);
	                if($inc_re){
	                    ...
	                    $bill['num'] = $tbnum;
	                    $bill['coinname'] = 'usdt';
	                    $bill['afternum'] = $minfo['usdt'] - $tbnum;
	                    $bill['type'] = 18;
	                    $bill['addtime'] = date("Y-m-d H:i:s",time());
	                    $bill['st'] = 2;
	                    $bill['remark'] = L('资金账户转保证金');
	                    
	                    M("bill")->add($bill);
	                    
	                    $this->ajaxReturn(['code'=>1,'info'=>L('划转成功')]);exit();
	                }else{
	                    $this->ajaxReturn(['code'=>2,'info'=>L('划转失败')]);exit();
	                }
	            }
	        }elseif($type == 2){
	            if($levermoney < $tbnum){
	                $this->ajaxReturn(['code'=>2,'info'=>L('保证金余额不足')]);exit();
	            }
	            
	            $order_count = M("leverorder")->where("uid = {$uid} and (status = 1 or status = 2)")->count();
	            if($order_count >= 1){
	                $this->ajaxReturn(['code'=>2,'info'=>L('有合约定单未结算，不能划转保证金')]);exit();
	            }
	            $dec_re = M("levermoney")->where(array('uid'=>$uid))->setDec('money',$tbnum);
	            if($dec_re){
	                $inc_re = M("user_coin")->where(array('userid'=>$uid))->setInc('usdt',$tbnum);
	                if($inc_re){
	                    //操作日志
	                    $bill['uid'] = $uid;
	                    $bill['username'] = $uinfo['username'];
	            ...
	            $this->ajaxReturn(['code'=>1,'info'=>L('参数错误')]);exit();
	        }
	...
	public function paytax(){
	    if($_POST){
	        ...
	        $cid = trim(I('post.cid'));
	        $zznum = trim(I('post.C2CofferBuy_quantity_input'));
	...
	public function getallzhehe(){
	    ...
	    $this->ajaxReturn(['code'=>2,'allzhehe'=>$allzhehe]);
	}
	...
	public function getmoneyusdz(){
	    ...
	    $re['zhe'] = $wallinfo['ukb'] * $usdzusdt + $wallinfo['ukbd'] * $usdzusdt;
	    $re['code'] = 1;
	...
	public function coininfo(){
	    ...
	    if($lowcoin == 'usdt'){
	       ...
	       $mlist = M("market")->where(array('name'=>"ukb_usdt"))->field("new_price")->find();
	       $data['kymoney'] = $minfo['ukb'];
	       $data['djmoney'] = $minfo['ukbd'];
	       $data['zhmoney'] = ($minfo['ukbd'] + $minfo['ukb']) * $mlist['new_price'];
	    }else{
	        $symbol = $lowcoin.'usdt';
	        $coinapi = "https://api.huobi.pro/market/history/kline?period=1day&size=1&symbol=".$symbol;
	        $result = $this->get_maket_api($coinapi);
	...
	public function paycoin(){
	    if($_POST){
	        ...
	        if($result){
	            $this->ajaxReturn(['code'=>1,'info'=> L('确认中')]);
	        }else{
